...
```    
不直接使用当前时间 currentTimeMillis 的原因是，在计算 newTokens 时，因为除法，某些时间片根本没有制造 Token。也就是说

newTokens = timeDelta * averageRate / rateToMsConversion

newTokens * rateToMsConversion / averageRate <= timeDelta

使用 currentTimeMillis 会跳过这些时间片，这是不对的。
