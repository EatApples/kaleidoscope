...
monitor 如何保证？答案很简单，使用锁。当一个线程想获得 monitor 时，首先会去抢占 monitor 锁。如果成功，则该线程能进入临界区执行操作。否则，该线程会阻塞，直到获得 monitor 的线程执行完为止。
...
举例来说，假设有2个线程，一个是生产者，一个是消费者。消费者先执行，调用 consume() 方法，发现 fullEntries = 0 (C0)，缓冲区没有数据，于是调用 wait(&full) (C1) 等待缓冲区满。随后生产者执行，发现其无需等待(P0)，则在缓冲区放入元素(P2)，fill 加一(P3)，fullEntries 加一(P4)，然后执行唤醒操作(P5)。在 Hoare 语义下，该生产者在执行唤醒操作之后，将不再继续执行。控制权转移到之前等待的消费者，其从等待中恢复(C1)，消费元素(C2)，以及之后的操作。只有当消费者返回之后，生产者才会获得控制权继续执行，直至结束。
...
### 6. 深入理解
为了深入理解为啥 Mesa 语义易于实现，我们先看下 Mesa 的 monitor 是如何实现的。在 Lampson 与 Redell 的工作中[LR80]，他们引入了3种不同的队列：就绪队列，monitor 锁队列，还有状态变量队列。一个线程，同一时刻，只能处于其中一种队列。注意，程序中可能有多个 monitor 类和状态变量，对于每一个 monitor，都有这3种队列。
