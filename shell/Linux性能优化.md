...
表示子进程已退出，但它的父进程还没有回收子进程占用的资源。短暂的僵尸进程不必理会，但长时间的僵尸进程通常是因为应用程序没有正常处理子进程的退出。解决方案：找出父进程，在父进程中解决
...
在你使用 ping 时，如果发现结果中的延迟并不大，而 ping 命令本身却很慢，不要慌，有可能是背后的 PTR 在搞鬼（PTR 反向地址解析的目的，是从 IP 地址反查出域名）
