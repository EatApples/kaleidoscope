...
CAP理论也在“在不可靠的分布式系统中，不可能同时满足安全性和活性”的框架之内。
...
在最终同步系统中，只要宕机的服务小于一半，就能达成一致；否则，就不可能。
...
最大努力一致（首先保证可用）：视频等流媒体，用户关心能否可以观看（可用），其次才是我看的是不是最新的（一致）。
...
层次分离，比如地图，在本市肯定先保证一致，本国保证可用就行了。
